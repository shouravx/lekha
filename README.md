...
│   ├── translator_engine.py    #   Argos Translate wrapper (singleton, cached)
...
Lekha never sends your documents, extracted text, or translated output anywhere. Once `scripts/download_models.py` has been run, the application has no further network dependency — translation (Argos Translate), OCR (PaddleOCR), and document generation (python-docx / ReportLab / PyMuPDF) all execute locally on your machine.
